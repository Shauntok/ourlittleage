    ...
    <main className="min-h-screen overflow-x-hidden bg-black px-5 py-20 text-white md:px-6 md:py-24">
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <header className="mb-12 md:mb-14">
          ...
          <p className="mt-5 max-w-xl text-sm leading-7 text-white/35 md:mt-6 md:leading-8">
        ...
        {initialPosts.length === 0 && (
          <div className="rounded-[2.2rem] border border-white/10 bg-white/[0.03] p-10 text-center backdrop-blur-2xl md:rounded-[2.5rem] md:p-14">
        ...
        <div className="space-y-5 md:space-y-6">
          {sortedPosts.map((post) => {
            ...
            return (
              <article
                key={post.id}
                className="min-w-0 overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.03] p-6 backdrop-blur-2xl transition-all duration-500 hover:border-white/20 hover:bg-white/[0.05] md:rounded-[2.2rem] md:p-8"
              >
                ...
                >
                  <p className="safe-pre line-clamp-5 text-[16px] leading-[2.15] text-white/78 md:text-[17px] md:leading-[2.2]">
                    {getExcerpt(post.content)}
                    {post.content.length > 260 ? "..." : ""}
                  </p>

                  <div className="mt-8 flex flex-wrap gap-3">
                    <span className="rounded-full border border-pink-500/20 bg-pink-500/[0.06] px-4 py-2 text-xs text-pink-100/60">
                      ❤️ 喜欢 {post.likeCount || 0}
                    </span>

                    <span className="rounded-full border border-blue-500/20 bg-blue-500/[0.06] px-4 py-2 text-xs text-blue-100/60">
